...
namespace DigitalSculpt
{
    ...
    void Mesh::computeOctree()
    {
        ...
        mesh_data_->_octree->resetNbFaces(mesh_data_->faces_count_/*getNbFaces()*/);
    ...
    Uint32Array Mesh::updateOctreeRemove(const Uint32Array* iFaces)
    {
        ...
        //Float32Array& fboxes = getFaceBoxes(); mesh_data_->_faceBoxes;
       // Uint32Array& facePosInLeaf = mesh_data_->_facePosInLeaf;
        ...
        {
            std::uint32_t idFace = iFaces->at(i);
            std::uint32_t idb = idFace * 6; // 这个face的包围盒，在face包围盒数组中的索引
            std::uint32_t idCen = idFace/* * 3*/; // 这个face的中心点，在face中心点数组中的索引
            OctreeCellPtr leaf = mesh_data_->_faceLeaf[idFace];
            SixElemArray& ab = leaf->_aabbSplit;

            float vx = mesh_data_->_faceCentersXYZ[idCen].x;
            float vy = mesh_data_->_faceCentersXYZ[idCen].y;
            float vz = mesh_data_->_faceCentersXYZ[idCen].z;

            if (vx <= leaf->_aabbSplit[0] || vy <= leaf->_aabbSplit[1] || vz <= leaf->_aabbSplit[2] || 
                vx > leaf->_aabbSplit[3] || vy > leaf->_aabbSplit[4] || vz > leaf->_aabbSplit[5])
            {
                // a face center has moved from its cell
                // 某面的中心点超出包围盒范围的话，就从本cell中将其移除出去
                facesToMove[acc++] = iFaces->at(i);
                //Uint32Array& facesInLeaf = leaf->_iFaces;
                std::uint32_t facesInLeafCount = leaf->_iFaces.size();

                if (facesInLeafCount > 0) // remove faces from octree cell
                {
                    std::uint32_t iFaceLast = leaf->_iFaces[facesInLeafCount - 1]; // 拿到”叶子中face数组“中最后那个face的id
                    std::uint32_t iPos = mesh_data_->_facePosInLeaf[idFace]; // 拿到给定face的位置索引
                    leaf->_iFaces[iPos] = iFaceLast; // 把这个“最后的face id”插入到那个要删掉的位置
                    mesh_data_->_facePosInLeaf[iFaceLast] = iPos;
                    leaf->_iFaces.pop_back();//把最尾部的那个移除掉 ，js代码是facesInLeaf.pop();
                }
            }
            else
            {
                // expands cell aabb loose if necessary
                leaf->expandsAabbLoose(
                    mesh_data_->_faceBoxes[idb + 0],
                    mesh_data_->_faceBoxes[idb + 1],
                    mesh_data_->_faceBoxes[idb + 2],
                    mesh_data_->_faceBoxes[idb + 3],
                    mesh_data_->_faceBoxes[idb + 4],
                    mesh_data_->_faceBoxes[idb + 5]);
            }
        }
    ...
    {
        //Float32Array& fc = getFaceCenters();mesh_data_->_faceCentersXYZ
        //Float32Array& fb = getFaceBoxes(); mesh_data_->_faceBoxes;
        //Uint32Array& facePosInLeaf = mesh_data_->_facePosInLeaf;
        //std::vector<OctreeCell*>& faceLeaf = mesh_data_->_faceLeaf;
        ...
        for (std::uint32_t i = 0; i < nbFacesToMove; ++i)
        {
            // add face to the octree
            std::uint32_t idFace = facesToMove->at(i);
            std::uint32_t idb = idFace * 6;
            float ibux = mesh_data_->_faceBoxes[idb + 0];
            float ibuy = mesh_data_->_faceBoxes[idb + 1];
            float ibuz = mesh_data_->_faceBoxes[idb + 2];
            float iblx = mesh_data_->_faceBoxes[idb + 3];
            float ibly = mesh_data_->_faceBoxes[idb + 4];
            float iblz = mesh_data_->_faceBoxes[idb + 5];
            ...
            OctreeCellPtr newleaf = root->addFace(idFace, ibux, ibuy, ibuz, iblx, ibly, iblz,
                mesh_data_->_faceCentersXYZ[idc + 0].x,
                mesh_data_->_faceCentersXYZ[idc + 1].y,
                mesh_data_->_faceCentersXYZ[idc + 2].z);

            if (newleaf != nullptr)
            {
                mesh_data_->_facePosInLeaf[idFace] = newleaf->_iFaces.size() - 1;
            ...
            { // failed to insert face in octree, re-insert it back
                ...
                mesh_data_->_facePosInLeaf[idFace] = facesInLeaf.size();
                facesInLeaf.push_back(facesToMove->at(i));
            }
        }
    }
    ...
    void Mesh::balanceOctree()
    {
        ...
        {
            OctreeCellPtr leaf = mesh_data_->_leavesToUpdate[i];
        ...
        Utils::ClearInstrutivePtrVector(mesh_data_->_leavesToUpdate);//mesh_data_->_leavesToUpdate.clear();// leavesToUpdate.length = 0;
    }
}