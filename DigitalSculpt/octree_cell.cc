...
namespace DigitalSculpt
{
    ...
    OctreeCell::OctreeCell(OctreeCellPtr parent)
    {
        InitSTACK();
    ...
    void OctreeCell::build(MeshPtr mesh)
    {
        int curStack = 1;
        //var stack = OctreeCell.STACK;
        STACK[0] = this;

        std::vector<OctreeCellPtr> leaves;// = new List<OctreeCell>();

        while (curStack > 0)
        {
            OctreeCellPtr cell = STACK[--curStack]; // 拿到当前栈顶再往下一个节点，然后赋值给变量cell
    ...
    void OctreeCell::constructLeaf(MeshPtr mesh)
    {
        /* try
         {*/
    ...
    void OctreeCell::constructChildren(MeshPtr mesh)
    {
        // 拿到用来进行分割用的AABB
        ...
        */
        ...
        OctreeCellPtr child0 = new OctreeCell(this); // 左下后
        OctreeCellPtr child1 = new OctreeCell(this); // 右下后
        OctreeCellPtr child2 = new OctreeCell(this); // 右下前
        OctreeCellPtr child3 = new OctreeCell(this); // 左下前
        OctreeCellPtr child4 = new OctreeCell(this); // 左上后
        OctreeCellPtr child5 = new OctreeCell(this); // 右上后
        OctreeCellPtr child6 = new OctreeCell(this); // 右上前
        OctreeCellPtr child7 = new OctreeCell(this); // 左上前
    ...
    Uint32Array OctreeCell::collectIntersectRay(const glm::vec3& vNear, const glm::vec3& eyeDir, Uint32Array& collectFaces, std::vector<OctreeCellPtr>* leavesHit)
    {
        int acc = 0;
        STACK[0] = this;
        int curStack = 1;
        ...
        while (curStack > 0)
        {
            OctreeCellPtr cell = STACK[--curStack];
    ...
    Uint32Array OctreeCell::collectIntersectSphere(const glm::vec3& vert, float radiusSquared, Uint32Array& collectFaces, std::vector<OctreeCellPtr>* leavesHit)
    {
        /* float vx = vert[0];
         float vy = vert[1];
         float vz = vert[2];*/
    ...
    OctreeCellPtr OctreeCell::addFace(int faceId, float bxmin, float bymin, float bzmin, float bxmax, float bymax, float bzmax, float cx, float cy, float cz)
    {
        STACK[0] = this;
        int curStack = 1;
    ...
    void OctreeCell::pruneIfPossible()
    {
        ...
        while (cell->_parent != nullptr)
        {
            ...
            for (int i = 0; i < 8; ++i)
            {
                OctreeCellPtr child = cell->_parent->_children[i];
