...
namespace kgl
{
	...
	void Camera::Move(CameraDirection dir)
	{
		if (camera_mode_ == FREE) 
		{
			switch (dir) {
			case UP:
				camera_position_delta_ += camera_up_ * camera_scale;
				break;
			case DOWN:
				camera_position_delta_ -= camera_up_ * camera_scale;
				break;
			case LEFT:
				camera_position_delta_ -= glm::cross(camera_direction_, camera_up_) * camera_scale;
				break;
			case RIGHT:
				camera_position_delta_ += glm::cross(camera_direction_, camera_up_) * camera_scale;
				break;
			case FORWARD:
				camera_position_delta_ += camera_direction_ * camera_scale;
				break;
			case BACK:
				camera_position_delta_ -= camera_direction_ * camera_scale;
				break;
	...
	void Camera::ChangeHeading(float degrees)
	{
		...
		if (camera_pitch_ > 90 && camera_pitch_ < 270 || (camera_pitch_ < -90 && camera_pitch_ > -270))
		{
			camera_yaw_ -= degrees;
		}
		else 
