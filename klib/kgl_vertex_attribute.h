...
{
    /*
    Index：        指定要配置顶点的哪一个属性。例如，在顶点着色器中使用layout(location = 0)定义了
                顶点的“位置（position）属性”的location值为0。因为希望把数据传递到这个顶点属性
                中，所以在这里要跟着shader代码里面的location的值，同样填0。

    Size：     下一个参数指定了每一个顶点的属性有多少个分量组成。例如，顶点的位置属性是vec3
                类型，它由3个float类型的数据组成。所以这里可以填3

    Type        本参数指定本待配置的“顶点的属性”所使用的数据的类型，例如顶点的位置属性使用32位
                浮点数的话。这里则指定值为GL_FLOAT。

    Normalized  本参数指定是否希望数据被标准化。如果设置为GL_TRUE，所有数据都会被映射到0（对于有
                符号型signed数据是-1）到1之间

    Stride      本参数即是指步长。本参数指定在连续的“顶点属性”描述字节之间，间隔有多少个字节。
    ...
    Pointer     本参数指定本顶点属性描述的顶点数据，在缓冲中起始位置的偏移量。
    */
